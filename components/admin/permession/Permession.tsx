...
import { validationSchemaPermession } from '@/helper/utils/validation/referance';
...
const Permession = () => {
    ...
    return (
        <Dialog onClose={() => setPermisstion({})} maxWidth="xs" fullWidth open={permisstion.open === "edit" || permisstion.open === "add"}>
            <div className='py-7'>
                <h1 className='text-center font-artin-black text-[#222] text-xl pt-3'> {permisstion.name === "edit" ? "ویراش" : "ایجاد"} سطح دسترسی</h1>
                <form onSubmit={formik.handleSubmit} className='p-5 flex flex-col gap-3'>
                    <Select label='نقش' getOptionValue={(option) => option._id} getOptionLabel={(option) => option.Name} formik={formik} name='Role' options={roles!} />
                    <Select label='Action' getOptionValue={(option) => option._id} getOptionLabel={(option) => option.Name} formik={formik} name='Action' options={actions!} />
                    <Select label='Resource' getOptionValue={(option) => option._id} getOptionLabel={(option) => option.Name} formik={formik} name='Resource' options={recources?.Resources!} />
                    <Select isDisabled={!formik.values.Resource._id} isMulti label='Attributes' isLoading={isLoading} getOptionValue={(option) => option._id} getOptionLabel={(option) => option.Name} formik={formik} name='Attributes' options={attributes?.Attributes} />

                    <button className='bg-[#0096f5] py-2 text-white rounded-lg font-artin-regular mt-10'>{permisstion.name === "edit" ? 'ویراش' : "ایجاد"}</button>
                </form>
            </div>
        </Dialog>
    )
