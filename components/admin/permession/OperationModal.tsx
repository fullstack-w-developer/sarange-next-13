          ...
          <form onSubmit={formik.handleSubmit} className="flex flex-col gap-2">
            {
              items.map((item, idx) => {
                ...
                if (item.Value === "Status") {
                  return <div key={idx} className="flex items-center gap-7">
                    <p className="font-artin-regular lg:text-[12px] block mb-1 pr-1 text-[#2F2F2F]">وضعیت :</p>
                    <Switch checked={formik.values[item.Value]} onChange={formik.handleChange} name={item.Value} color="warning" />
                  </div>

                }

                return <Input key={idx} formik={formik} label={item.Name} name={item.Value} />
