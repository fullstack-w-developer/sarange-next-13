...
const EditUser = () => {
    ...
    const editUserByAdmin = async (e: FormData) => {
        ...
        await actionEditUserByAdmin(operationUser.info?._id!, data).then(() => {
                ...
                <div className="p-2 px-4 pt-4">
                    <form action={editUserByAdmin} className='flex flex-col gap-2'>
                        {operationUser.operation.Attributes.map((attr: any) => {
                            ...
                            return componentConfig ? (
                                componentConfig.type === "text" ?
                                    // @ts-ignore
                                    <Input defaultValue={operationUser?.info[attr.Value]} key={attr._id} label={componentConfig.label} name={componentConfig.name} />
                        ...
                        <div className="flex items-center mt-7 my-4 px-4 gap-10">
                            ...
                            {/* @ts-ignore */}
                            <button type='submit' className="w-full bg-green-500 text-white border border-[#e1e1e1] py-[10px] rounded-lg font-artin-bold">
                                ویرایش کاربر
                            </button>
                        </div>
                    </form>
