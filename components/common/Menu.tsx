...
const Menu = ({ menu }: Props) => {
    ...
    return (
        <div className="flex items-center rounded-t-[24px] justify-center  fixed w-full bottom-0 h-[80px] menu">
            {menu.map((menu, i) => (
                <Link href={menu.url} className={`flex relative flex-1 h-full flex-col items-center justify-center gap-2`} key={i}>
                    ...
                    {
                        pathname === menu.url &&
                        <span className="active-menu block absolute bottom-0"></span>
                    }
                </Link>
            ))}
        </div>
