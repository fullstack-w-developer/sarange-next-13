...
const Users = ({ list }: Props) => {
    ...
    return (
        <div className="flex-1 w-full mb-20">
                                ...
                                <div className="flex gap-3 items-center justify-center">
                                    {
                                        list.operation.map((operation, idx) => (
                                            <button onClick={() => toggle_opration_user({ open: operation.Action, info: user, operation: operation })} key={idx} className={`flex items-center gap-1 text-[14px]  px-3 py-[8px] rounded-lg text-white ${operation.Action === "ویرایش" ? "bg-green-500" : "bg-red-500"}`}>
                                                <p className="pt-[1px]">{operation.Action}</p>
                                                {operation.Action === "ویرایش" &&
                                                    <BiEditAlt size={14} color="#fff" />
                                                }
                                                {operation.Action === "حذف" &&
                                                    <AiFillDelete size={14} color="#fff" />
                                                }
                                            </button>
                                        ))
                                    }
            ...
            />
            {
                operationUser.open === "حذف" &&
                <DeleteUserByAdmin />
            }
            {
                operationUser.open === "ویرایش" &&
                <EditUser />
            }
        </div>
    );
