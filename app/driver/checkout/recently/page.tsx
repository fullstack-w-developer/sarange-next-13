          ...
          historyCheckout.map((item, idx) => (
            <div className='flex items-center justify-between font-artin-regular border-b pb-4' key={idx}>
              <div className='space-y-5'>
                <p><span>وضعیت: </span>{item.status}</p>
                <p><span>مبلغ: </span>{item.Amount.toLocaleString()}</p>
              </div>
              <div className='space-y-5'>
                <p><span>شماره شبا: </span>{item.WithdrawalTo}</p>
                {/* @ts-ignore */}
                <p><span>تاریخ تسویه: </span>{new Date(item.createdAt).toLocaleDateString("fa-IR", options)}
                </p>
              </div>
            </div>
          ))
